...
- **Session snapshot sanitization**: `sanitizeForkSnapshot()` strips sliding prompts, reasoning artifacts, strategic hints, and `batch_read` tool calls from assistant messages, and compresses prior flow tool results into compact `CompressedFlowResult` context maps before forking.
