...
import { compressToolResults, compressFlowToolCallArgs, depthToPolicy, sanitizeForkSnapshot, stripOrchestratorNarrative, stripUserMissionManifestos, stripConversationalAcks } from "../src/snapshot/snapshot.js";
...
describe("stripOrchestratorNarrative", () => {
	it("replaces low-signal assistant narration under 500 chars with placeholder", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "text", text: "Okay, let me think about this problem and figure out the best approach to take here." }],
				},
			},
		]);

		const result = stripOrchestratorNarrative(snapshot);
		const entries = parseSnapshot(result);
		const assistant = entries.find((e) => e?.message?.role === "assistant");
		expect(assistant.message.content).toBe("[orchestrator:thinking]");
	});

	it("preserves assistant messages with file paths", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "text", text: "Look at src/index.ts for the entry point." }],
				},
			},
		]);
		...
		const entries = parseSnapshot(result);
		const assistant = entries.find((e) => e?.message?.role === "assistant");
		expect(assistant.message.content[0].text).toBe("Look at src/index.ts for the entry point.");
	});

	it("preserves assistant messages with code blocks", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "text", text: "Use ```npm test``` to verify changes." }],
				},
			},
		]);
		...
		const entries = parseSnapshot(result);
		const assistant = entries.find((e) => e?.message?.role === "assistant");
		expect(assistant.message.content[0].text).toBe("Use ```npm test``` to verify changes.");
	});

	it("preserves assistant messages with tool calls", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					...
					content: [
						...
						{ type: "toolCall", name: "batch", id: "tc-1" },
					],
				},
			},
		]);
		...
		const entries = parseSnapshot(result);
		const assistant = entries.find((e) => e?.message?.role === "assistant");
		expect(assistant.message.content[1].type).toBe("toolCall");
	});

	it("preserves assistant messages at exactly 500 chars", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
	...
	it("preserves session header", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1", systemPrompt: "test" },
		]);

		const result = stripOrchestratorNarrative(snapshot);
		const entries = parseSnapshot(result);
		expect(entries[0].type).toBe("session");
		expect(entries[0].systemPrompt).toBe("test");
	});

	it("includes token count in placeholder when usage is present", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "text", text: "Hmm, interesting." }],
		...
		const entries = parseSnapshot(result);
		const assistant = entries.find((e) => e?.message?.role === "assistant");
		expect(assistant.message.content).toBe("[orchestrator:thinking — 123 tokens]");
	});
});

describe("stripUserMissionManifestos", () => {
	it("replaces large user JSON mission manifestos with placeholder", () => {
		const manifesto = JSON.stringify({
			mission: "Refactor everything",
			stages: [{ name: "analyze" }, { name: "execute" }],
			execution_directive: "Be thorough",
		}) + " ".repeat(900); // pad to >800 chars
		...
		const result = stripUserMissionManifestos(snapshot);
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("[user:mission — see activation prompt]");
	});

	it("preserves user messages with file paths", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "user",
					content: "A".repeat(900) + " Check /src/index.ts for the bug.",
				},
			},
		]);
		...
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("A".repeat(900) + " Check /src/index.ts for the bug.");
	});

	it("preserves user messages under 800 chars", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
		...
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe('{"mission":"test"}');
	});
	...
	it("preserves already-collapsed placeholders", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
		...
		expect(users[0].message.content).toBe("[user:ack]");
		expect(users[1].message.content).toBe("[orchestrator:thinking]");
	});
});

describe("stripConversationalAcks", () => {
	it("replaces short conversational user messages with placeholder", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "user",
					content: "that's good now proceed",
				},
			},
		]);

		const result = stripConversationalAcks(snapshot);
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("[user:ack]");
	});
	...
	it("preserves user messages with file paths", () => {
		...
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("check src/index.ts");
	});

	it("preserves user messages with commands", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
		...
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("run `npm test`");
	});

	it("preserves user messages at exactly 100 chars", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
		...
		const entries = parseSnapshot(result);
		const user = entries.find((e) => e?.message?.role === "user");
		expect(user.message.content).toBe("A".repeat(100));
	});
	...
	it("preserves already-collapsed placeholders from earlier passes", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
			...
			},
			{
				...
				message: {
					role: "user",
					content: "[orchestrator:thinking — 42 tokens]",
				},
			},
		]);
		...
		const entries = parseSnapshot(result);
		const users = entries.filter((e) => e?.message?.role === "user");
		expect(users[0].message.content).toBe("[user:mission — see activation prompt]");
		expect(users[1].message.content).toBe("[orchestrator:thinking — 42 tokens]");
	});
});

describe("sanitizeForkSnapshot — narrative passes integration", () => {
	it("applies all three narrative passes in the pipeline", () => {
		const snapshot = makeSnapshot([
			{ type: "session", id: "sess-1" },
			{
				type: "message",
				message: {
					role: "assistant",
					content: [{ type: "text", text: "Let me analyze the requirements carefully." }],
				},
			},
			{
				type: "message",
			...
			},
			{
				...
				message: {
					role: "user",
					content: JSON.stringify({ mission: "X", stages: [], execution_directive: "Y" }) + " ".repeat(1000),
				},
			},
		]);
		...
		const { result, passesApplied } = sanitizeForkSnapshot(snapshot, new Map(), { depth: 1 });
		expect(result).toBeDefined();
		expect(passesApplied).toContain("stripOrchestratorNarrative");
		expect(passesApplied).toContain("stripUserMissionManifestos");
		expect(passesApplied).toContain("stripConversationalAcks");
	});
});
